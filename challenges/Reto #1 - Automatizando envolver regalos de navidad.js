...
Cada regalo es un string. Necesitamos que la máquina envuelva cada
regalo en papel de regalo y lo coloque en un array de regalos envueltos.
...
function wrapping(gifts) {
  return gifts.map(gift => {
    const tape = '*'
    const upNDown = (tape.repeat(gift.length) + '**')
    const between = ('\n*' + gift + '*\n')
    return upNDown + between + upNDown
  })
}